...
owner_id = 199776748 #ID Вк
owner_name = "Даниил" #Ваше имя
owner_tg = "thedeaddan" #Ваш ник в TG
...
try:
	...
	blacklist = ["2905049","45419041","95273977","264429430","105897022"]#Массив тех, кому нельзя писать
		...
		for Dialog in GetDialogs():#Перебираем диалоги
			...
			if ids[:2] != "20" and ids[:1] != "-" and ids not in blacklist: #Если диалог с человеком, и он не в чёрном списке
				if "!счс" in text.lower() and from_id == owner_id: #Команда на добавления в чёрный список
					...
					vk.method("messages.send",{"random_id":0,"peer_id":ids,"message":"Секретарь больше не будет отвечать в этом диалоге"}) #Уведомили об этом
				try:
				...
				if Unreaded: #Если не прочитан
					if ids in Writed: #Если бот уже писал человеку
						...
						if ids not in Writed:#Если человека нет в массиве, добавляем
							...
							vk.method("messages.send",{"random_id":0,"peer_id":owner_id,"message":str(str(Writed).replace("'","").replace("[","").replace("]",""))+"|"+str(str(blacklist).replace("'","").replace("[","").replace("]",""))})#Отправляем новый пакет данных
		time.sleep(1)

	except KeyboardInterrupt:
		print("\nСекретарь завершает свою работу") #Если бота закрыли, выводим сообщение 
		vk.method("messages.send",{"random_id":0,"peer_id":owner_id,"message":str(str(Writed).replace("'","").replace("[","").replace("]",""))+"|"+str(str(blacklist).replace("'","").replace("[","").replace("]",""))}) #Отправляем последнюю версию массива себе в лс
